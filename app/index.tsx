...
function Home() {
  interface NavigationProps {
    // Define any props you may need (if applicable)
  }
  const Navigation: React.FC<NavigationProps> = () => {
    ...
    const showContext = (context: string) => {
      // Handle the context (could be routing or state management logic)
    ...
    return (
      <div className="Navigation">
        <button className="nav-toggle" onClick={() => setIsAboutOpen(!isAboutOpen)}>
          &#9776;
        </button>
        <ul className="nav-menu">
          <li className="nav-item">
            <a href="#" onClick={() => showContext('home')}>
              <i className="fa fa-home" id="home-icon"></i>
            </a>
          </li>
          <li className="nav-item">
            <div className="nav-link" onClick={() => setIsAboutOpen(!isAboutOpen)}>
              <a href="#">Giới thiệu</a>
              <i className="fa-solid fa-sort-down"></i>
            </div>
            {isAboutOpen && (
              <div className="nav-item-hidden">
                <a href="#" onClick={() => showContext('about_1')}>Mục đích và phạm vi của tạp chí</a>
                <a href="#" onClick={() => showContext('about_2')}>Tần suất xuất bản</a>
                <a href="#" onClick={() => showContext('about_3')}>Biên tập và đạo đức xuất bản</a>
                <a href="#" onClick={() => showContext('about_4')}>Chính sách chung và các nguyên tắc</a>
                <a href="#" onClick={() => showContext('about_5')}>Tài trợ tạp chí</a>
              </div>
            )}
          </li>
          <li className="nav-item">
            <div className="nav-link" onClick={() => setIsGuidelineOpen(!isGuidelineOpen)}>
              <a href="#">Hướng dẫn</a>
              <i className="fa-solid fa-sort-down"></i>
            </div>
            {isGuidelineOpen && (
              <div className="nav-item-hidden">
                <a href="#" onClick={() => showContext('guideli_tacgia')}>Hướng dẫn tác giả</a>
                <a href="#" onClick={() => showContext('guideli_phanbien')}>Hướng dẫn phản biện</a>
              </div>
            )}
          </li>
          <li className="nav-item">
            <a href="#" onClick={() => showContext('editing')}>Biên tập</a>
          </li>
          <li className="nav-item">
            <a href="#" onClick={() => showContext('archiving')}>Lưu trữ</a>
          </li>
          <li className="nav-item">
            <a href="#" onClick={() => showContext('submission')}>Gửi bài</a>
          </li>
          <li className="nav-item">
